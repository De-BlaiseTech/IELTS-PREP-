...
function render(){if(!state.user&&state.page!=="login"&&state.page!=="register")state.page="login";const pages={dashboard,practice,mock,listening:()=>skillPage("listening"),reading:()=>skillPage("reading"),writing,speaking,progress,profile,exam,login,register};document.getElementById("app").innerHTML=(pages[state.page]||dashboard)();bind();ensureGlobalLoader();if(state.page==="listening")setupListeningAudioPlayer()}
function toggleMenu(){const m=document.getElementById("mobileNav"),b=document.getElementById("mobileMenuButton");if(!m)return;const open=m.classList.toggle("hidden")===false;if(b)b.setAttribute("aria-expanded",String(open));} function closeMenu(){const m=document.getElementById("mobileNav"),b=document.getElementById("mobileMenuButton");m?.classList.add("hidden");if(b)b.setAttribute("aria-expanded","false")}
...
async function playListeningAudio(testId, section){
  ...
  try{
    ...
    if(!r.ok || !data.url){
      throw new Error(data.message||`Audio service returned ${r.status}.`);
    }
    ...
    if(status) status.textContent=e.message||"Unable to load audio.";
    alert(e.message||"Unable to load listening audio.");
  }finally{
...
function setupListeningAudioPlayer(){
  ...
  player.addEventListener("loadstart",()=>{
    if(status) status.textContent="Connecting to Backblaze B2…";
  });
  player.addEventListener("loadedmetadata",()=>{
    if(status) status.textContent="Audio loaded. Tap ▶ to play.";
  });
  player.addEventListener("playing",()=>{
    if(status) status.textContent="Listening audio is playing.";
  });
  player.addEventListener("pause",()=>{
    if(status && player.currentTime>0 && !player.ended) status.textContent="Audio paused.";
  });
  player.addEventListener("ended",()=>{
    if(status) status.textContent="Section audio finished.";
  });
  player.addEventListener("error",()=>{
    const code=player.error?.code;
    const detail=code?` (media error ${code})`:"";
    if(status) status.textContent=`The audio could not be played${detail}.`;
  });
}
...
function writingVisual(task){
  const svg=(body,caption)=>`<div class="writing-visual"><div class="visual-title">${caption}</div><div class="visual-scroll">${body}</div><div class="visual-note">Study the visual carefully and report the main features; exact figures are provided for practice.</div></div>`;
  if(task.visualType==='bar') return svg(`<svg viewBox="0 0 720 330" role="img" aria-label="Bar chart showing weekly exercise hours"><line x1="70" y1="270" x2="680" y2="270" stroke="#6b7f93"/><line x1="70" y1="30" x2="70" y2="270" stroke="#6b7f93"/><text x="18" y="38" font-size="13">Hours</text>${[['18–29',4,7,11],['30–49',3,6,9],['50+',2,4,6]].map((d,i)=>{const x=130+i*180;return `<text x="${x+30}" y="300" font-size="13">${d[0]}</text><rect x="${x}" y="${270-d[1]*18}" width="35" height="${d[1]*18}"/><rect x="${x+45}" y="${270-d[2]*18}" width="35" height="${d[2]*18}"/><rect x="${x+90}" y="${270-d[3]*18}" width="35" height="${d[3]*18}"/>`}).join('')}<text x="510" y="40" font-size="12">Gym</text><text x="510" y="58" font-size="12">Walking</text><text x="510" y="76" font-size="12">Cycling</text></svg>`,`Weekly exercise hours by age group`);
  if(task.visualType==='line') return svg(`<svg viewBox="0 0 720 330" role="img" aria-label="Line graph showing library visitors"><line x1="70" y1="270" x2="680" y2="270" stroke="#6b7f93"/><line x1="70" y1="30" x2="70" y2="270" stroke="#6b7f93"/><polyline points="100,210 220,180 340,205 460,125 580,95 650,115" fill="none" stroke="#315d82" stroke-width="4"/><polyline points="100,235 220,225 340,195 460,175 580,145 650,130" fill="none" stroke="#9a6a2f" stroke-width="4"/>${[['Jan',100],['Mar',220],['May',340],['Jul',460],['Sep',580],['Nov',650]].map(x=>`<text x="${x[1]-12}" y="292" font-size="12">${x[0]}</text>`).join('')}<text x="515" y="48" font-size="12">In-person</text><text x="515" y="66" font-size="12">Online</text></svg>`,`Library visits: in-person and online, January–November`);
  if(task.visualType==='pie') return svg(`<svg viewBox="0 0 720 330" role="img" aria-label="Pie chart of household energy use"><circle cx="210" cy="165" r="105" fill="none" stroke="#315d82" stroke-width="70" stroke-dasharray="231 429"/><circle cx="210" cy="165" r="105" fill="none" stroke="#9a6a2f" stroke-width="70" stroke-dasharray="147 513" stroke-dashoffset="-231"/><circle cx="210" cy="165" r="105" fill="none" stroke="#5f7f67" stroke-width="70" stroke-dasharray="84 576" stroke-dashoffset="-378"/><text x="350" y="105" font-size="14">Heating — 55%</text><text x="350" y="145" font-size="14">Lighting — 20%</text><text x="350" y="185" font-size="14">Appliances — 15%</text><text x="350" y="225" font-size="14">Other — 10%</text></svg>`,`Household energy consumption`);
  if(task.visualType==='map') return svg(`<svg viewBox="0 0 720 330" role="img" aria-label="Map of a town centre before and after redevelopment"><rect x="60" y="35" width="260" height="245" fill="#eef2f5" stroke="#6b7f93"/><rect x="400" y="35" width="260" height="245" fill="#eef2f5" stroke="#6b7f93"/><path d="M75 150 H305 M185 45 V270" stroke="#6b7f93" stroke-width="12"/><path d="M415 150 H645 M530 45 V270" stroke="#6b7f93" stroke-width="12"/><rect x="105" y="70" width="70" height="45"/><rect x="205" y="190" width="80" height="45"/><circle cx="235" cy="100" r="24"/><rect x="440" y="70" width="75" height="55"/><rect x="555" y="185" width="70" height="50"/><circle cx="500" cy="190" r="30"/><path d="M535 75 C575 105 575 150 535 180" fill="none" stroke="#315d82" stroke-width="8"/><text x="145" y="315" font-size="13">Before</text><text x="515" y="315" font-size="13">After redevelopment</text></svg>`,`Town-centre redevelopment: before and after`);
  return '';
}
const WRITING_TASK1S=[
 {title:'Academic Writing Task 1 — Exercise',min:150,visualType:'bar',prompt:'The bar chart below compares the number of hours per week that three age groups spent on gym exercise, walking and cycling. Summarise the information by selecting and reporting the main features, and make comparisons where relevant.'},
 {title:'Academic Writing Task 1 — Library visits',min:150,visualType:'line',prompt:'The line graph below shows the number of people visiting a city library in person and using its online service from January to November. Summarise the information by selecting and reporting the main features, and make comparisons where relevant.'},
 {title:'Academic Writing Task 1 — Energy use',min:150,visualType:'pie',prompt:'The pie chart below shows how a household used its energy in four categories. Summarise the information by selecting and reporting the main features, and make comparisons where relevant.'},
 {title:'Academic Writing Task 1 — Town redevelopment',min:150,visualType:'map',prompt:'The maps below show a town centre before and after redevelopment. Summarise the information by selecting and reporting the main features, and make comparisons where relevant.'}
];
function writing(){
